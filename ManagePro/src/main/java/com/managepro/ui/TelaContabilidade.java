...
public class TelaContabilidade {
	...
	public TelaContabilidade() throws ClassNotFoundException, SQLException {
	...
	private void initialize() {
		...
		// Exibir gráficos por padrão ao inicializar
	    try {
	        exibirGraficosPorDefault();
	    } catch (ExcecaoDoSistema e1) {
	        e1.printStackTrace();
	...
	private void exibirGraficosPorDefault() throws ExcecaoDoSistema {
        ...
        try {
            ...
            JFreeChart chartColuna = ChartFactory.createBarChart(
                "Estatísticas",
                "Categoria",
                "Valor",
                dataset,
		...
		private void validarData() throws ExcecaoDoSistema { 
		...
		private void gerarGraficos(Date dataInicial, Date dataFinal) throws ExcecaoDoSistema {
		    try {
		        ...
		        List<Estatistica> estatisticas = estatisticaService.findByData(sqlDate, sqlDate2);
		        
		        
		        // Verifique se a lista não está vazia
		        if (estatisticas.isEmpty()) {
		            throw new ExcecaoDoSistema("Nenhum dado encontrado para o período especificado.");
		        }
