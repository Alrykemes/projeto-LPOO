...
VALUES("Administrador MANAGEPRO", "123.456.789-12", "ADMINISTRADOR", 10000.00, '2024-08-12', "admin", "123456");

INSERT INTO funcionario (nome, cpf, cargo, salario, data_admissao, usuario, senha) 
VALUES("Vendedor Teste", "123.456.789-12", "VENDEDOR", 1352.30, '2024-08-12', "vendedor", "123456");
...
BEGIN
    -- Verifica se j� existe uma linha com a data fornecida
    IF EXISTS (SELECT 1 FROM estatistica WHERE data = dataEscolhida) THEN
        -- Se a linha j� existe, atualiza a linha
        UPDATE estatistica
        SET 
    ...
    ELSE
        -- Se a linha n�o existe, insere uma nova linha
        INSERT INTO estatistica (quantidade_produtos_vendidos, quantidade_vendas, total_ganho, data)
        SELECT 
...
BEGIN
    ...
    WHERE id = 1; -- Ajuste o WHERE conforme necess�rio
END$$
