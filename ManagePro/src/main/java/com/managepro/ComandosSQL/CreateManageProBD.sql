...
INSERT INTO produto (nome, preco, quantidade, marca, fornecedor, validade) 
VALUES("Arroz", "7.32", 20, "Emo��es", 'Cadan Distribui��o', '2027-12-18');

INSERT INTO produto (nome, preco, quantidade, marca, fornecedor, validade) 
VALUES("Feij�o", "5.29", 20, "Turquesa", 'Cadan Distribui��o', '2026-07-26');

INSERT INTO produto (nome, preco, quantidade, marca, fornecedor, validade) 
VALUES("Azeite de Oliva", "46.90", 20, "Gallo", 'Cadan Distribui��o', '2026-03-17');

# Trigger para n�o permitir Troco e Valor Recebido ser nullo quando a forma de pagamento for dinheiro.
DELIMITER //
...
BEGIN
    IF NEW.forma_pagamento = 'DINHEIRO' THEN
        IF NEW.valor_recebido IS NULL OR NEW.troco IS NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro: Valor recebido e troco devem ser preenchidos quando o m�todo de pagamento for DINHEIRO.';
        END IF;
    ELSE
        IF NEW.valor_recebido IS NOT NULL OR NEW.troco IS NOT NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro: Valor recebido e troco devem ser nulos quando o m�todo de pagamento n�o for DINHEIRO.';
        END IF;
    END IF;
END//
...
# Procedure de atualiza��o da tabela de Estatistica
